...
{
/*    // construct with areaSymmTensorField, PJS
    if(typeid(tmp<faMatrix<Type> >) == typeid(areaSymmTensorField))             // funktioniert nicht
    {
    ...
    else if( typeid(tmp<faMatrix<Type> >) != typeid(areaSymmTensorField))        // soll eigentlich nur else sein, aber typid() drüber funktiniert nicht
    {
    ...
    Info << "############################## Type name of tmp<faMatrix<Type> > is:" << typeid(tmp<faMatrix<Type> >).name() << endl;
